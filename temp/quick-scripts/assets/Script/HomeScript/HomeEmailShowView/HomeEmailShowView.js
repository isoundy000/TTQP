(function() {"use strict";var __module = CC_EDITOR ? module : {exports:{}};var __filename = 'preview-scripts/assets/Script/HomeScript/HomeEmailShowView/HomeEmailShowView.js';var __require = CC_EDITOR ? function (request) {return cc.require(request, require);} : function (request) {return cc.require(request, __filename);};function __define (exports, require, module) {"use strict";
cc._RF.push(module, 'c89ceLAftJNJ54NFp6Cqmhu', 'HomeEmailShowView', __filename);
// Script/HomeScript/HomeEmailShowView/HomeEmailShowView.js
...
cc.Class({
        ...
        if (this.emailData.length == 0) {
            ...
            for (var i = 0; i < this.emailData.length; i++) {
                ...
                if (this.itemIdx != 2) {
                    ...
                    item.getComponent('HomeEmailItem').init(itemDic, i, function (itemData, clickIdx) {
    ...
    // 初始化数据配置数组
    onInitReqParamsConfigs: function onInitReqParamsConfigs() {
        this.reqParamsConfigs = [{ "page": 0, list: [] }, { "page": 0, list: [] }, { "page": 0, list: [] }];
    },
    ...
    /****************************************  本类接口相关  ****************************************/
    // 获取 邮件列表 数据
    onGetEmailData: function onGetEmailData(typeIdx) {
        var idx = 2;
        // 决定邮件类型
        if (typeIdx == 0) idx = 2;
        if (typeIdx == 1) idx = 1;
    ...
    // 发送 阅读邮件 接口
    onReadEmailRequest: function onReadEmailRequest(emailID) {
        var params = {
            "mailId": emailID
        };
    ...
    // 发送 删除邮件 接口
    onDelEmailRequest: function onDelEmailRequest(emailID) {
        var params = {
            "mailId": emailID
        };
    ...
    /****************************************  本类接口回调相关  ****************************************/
    // 获取服务器返回的 邮件列表 信息
    emailDataBlock: function emailDataBlock(data) {
        if (!data) return;
    ...
    // 阅读邮件接口 回调
    emailReadBlock: function emailReadBlock(data) {
    ...
    // 删除邮件回调
    emailDelBlock: function emailDelBlock(data) {
    ...
    // 配置邮件类型按钮
    onConfigEmailTypeBtn: function onConfigEmailTypeBtn(isFriend, isSystemEmail, isSystemMsg) {
     ...
     * 处理点击查看邮件详情后的处理
     */
    onClickGoDetailBtnConfig: function onClickGoDetailBtnConfig(itemData, clickIdx) {
        this.clickIdx = clickIdx;
        if (itemData.isRead.toString() == '0') {

            if (parseFloat(itemData.gold) > 0 && parseInt(itemData.isRec) == 0) {} else {
                // 发送 邮件阅读 接口
        ...
        var self = this;
        // 弹出详情弹窗
        this.emailDetailView.getComponent('HomeEmailDetailView').onShowView(true, this.itemIdx == 0, itemData, function (menusIdx, currentItemData) {
     ...
     */
    onEmailLoadMoreHandle: function onEmailLoadMoreHandle(scrollview, eventType) {
...
cc._RF.pop();
        ...
        //# sourceMappingURL=HomeEmailShowView.js.map
